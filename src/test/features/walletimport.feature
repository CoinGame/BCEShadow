Feature: A Nubits wallet is imported and the BKS for the private keys become available
  ...
  Scenario: Wallet is imported and BKS become available
    ...
    And node "Alice" sends "5" BlockShares to raw address "tBP2ojyVr14hamNWCrZ6UEviQMu2R1aTRW"
    And node "Alice" sends "5" BlockShares to raw address "tSHHnzgLYfoP5WhJaGqpM1SKxrmvfxvwKB"
    And node "Alice" sends "50" BlockShares to raw address "tV2koTDQruNoNvQXGRSemAtvV7yEFQDS3m"
    And node "Alice" sends "50" BlockShares to raw address "tQhRvXAwerWXxW51TEqAEf9mWCjRBdMq12"
    And node "Alice" sends "290" BlockShares to raw address "uRSEEuREXSscqosTfjcUk8M845tBizN4py"
    ...
    Then node "Bob" should have a balance of "110" BlockShares

    When node "Bob" imports encrypted wallet "data/walletS.enc.dat" with password "password"
    Then node "Bob" should have a balance of "210" BlockShares

    When node "Bob" imports wallet "data/walletS.multisig.dat"
    Then node "Bob" should have a balance of "500" BlockShares